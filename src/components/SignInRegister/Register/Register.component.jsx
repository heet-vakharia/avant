...
const Register = ({ setUser, setIsUserLogIn }) => {
  ...
  const onRegister = async () => {
    setIsLoading(true);
    if (userid && password & (isEmail(email) === true)) {
      setIsLoading(true);
      await fetch("https://young-temple-47679.herokuapp.com/register", {
        method: "post",
      ...
      setIsLoading(false);
      setErrMsg("PLz filled up all details correctly");
    }
  };
  ...
  return (
    <section className="loginRegister">
      <div className="form">
        ...
        <label htmlFor="email" className="form--Email-label form--item-label">
          <input
            type="email"
            name="Email"
            id="email"
        ...
        <button type="button" className="form__btn" onClick={onRegister}>
